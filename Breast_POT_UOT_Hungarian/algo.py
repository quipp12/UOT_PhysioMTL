    ...
    while Matched.sum() != Size:
        ...
        if set(S_neighbor) != set(T):
            ...
            if not np.where(Matched[:, y])[0].size:  # free
                ycorre = list(set(np.where(Cost_temp[:, y] == 0)[0]) - set(np.where(Matched[:, y] == 1)[0]) - (
                            set(range(Size)) - set(S)))[0]  # ycorre should in S
                ...
                while terminate != free:  # when the initial point is free vertex, stop tracing
                    ...
                    Matched_augment_path.append(
                        Matched_path[np.where(np.array(Matched_path)[:, 0] == Unmatched_augment_path[-1][0])[0][0]])
                    Unmatched_augment_path.append(
                        Unmatched_path[np.where(np.array(Unmatched_path)[:, 1] == Matched_augment_path[-1][1])[0][0]])
...
def revised_Hungarian(Cost):
    ...
    if Matched.sum() == Size2 and all(
            Matched.sum(axis=1) == (Size1 * np.ones(Size1))):  # judge if it is a perfect psedo-matching
        return Matched
    ...
    while Matched.sum() != Size2 and any(Matched.sum(axis=1) != (Size1 * np.ones(Size1))):
        ...
        if len(list(set(S_neighbor) - set(T))):
            ...
            if not np.where(Matched[:, y])[0].size:  # free
                ycorre = list(set(np.where(Cost_temp[:, y] <= 1e-8)[0]) - set(np.where(Matched[:, y] == 1)[0]) - (
                            set(range(Size1)) - set(S)))[0]  # ycorre should in S
    ...
    while Matched.sum() != Size:
        ...
        if set(S_neighbor) != set(T):
            ...
            if not np.where(Matched[:, y])[0].size:  # free
                ...
                ycorre = list(set(np.where(tempcost <= 1e-8)[0]) - set(np.where(Matched[:, y] == 1)[0]) - (
                            set(range(Size)) - set(S)))[0]  # ycorre should in S
            ...
            if (mark == 1) & (np.where(Matched[:, y])[0].size):  # matched
                ...
                temp2 = np.vstack((temp[np.where(~np.isnan(Slack))[0]], Slack[np.where(~np.isnan(Slack))[0]])).min(
                    axis=0)
    ...
    while Matched.sum() != Size2 and any(Matched.sum(axis=1) != (Size1 * np.ones(Size1))):
        ...
        if len(list(set(S_neighbor) - set(T))):
            ...
            if (mark == 1) & (np.where(Matched[:, y])[0].size):  # psedo-matched
                ...
                temp2 = np.vstack(
                    (tempcost[0][np.where(~np.isnan(Slack))[0]], Slack[np.where(~np.isnan(Slack))[0]])).min(axis=0)
